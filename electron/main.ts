...
// Wayland & GPU Hardware Acceleration
...
function getMusicFolder(): string {
  const custom = "/home/darnell/Music";
...
function loadDb(): LibraryData {
  try {
    if (fs.existsSync(DB_FILE)) {
      const data: LibraryData = JSON.parse(fs.readFileSync(DB_FILE, "utf-8"));
  ...
  const defaultMusic = getMusicFolder();
  return {
...
async function scanFolders(folders: string[], sendProgress?: (p: ScanProgress) => void): Promise<Track[]> {
  ...
  for (const p of filePaths) {
    ...
    if (sendProgress && (done % 5 === 0 || done === filePaths.length)) {
...
// Custom protocol for high-performance audio streaming
...
function registerCadenceProtocol() {
  protocol.handle("cadence", async (request) => {
    try {
      const rawUrl = request.url.replace(/^cadence:\/\//, "");
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle("library:load", async () => {
    const db = loadDb();
    if (db.tracks.length === 0 && db.folders.length > 0) {
      // Auto-scan initial music folder on first launch
